...
imdb_longer %>%
  ...
  theme_minimal() +
  scale_x_continuous(breaks = c(0, 1000000000, 2000000000), labels = c("$0", "$1B", "$2B"))
```
...
imdb_income_country_regression <- linear_reg() %>%
  fit(us_rating ~ worlwide_gross_income + country, data = imdb_country)
  tidy(imdb_income_country_regression)
  
imdb_income_regression <- linear_reg() %>%
  fit(us_rating ~ worlwide_gross_income, data = imdb_country)
  
glance(imdb_income_country_regression)$r.squared
glance(imdb_income_regression)$r.squared
```
